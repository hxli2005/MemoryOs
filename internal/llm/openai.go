...
// OpenAIClient OpenAI LLM 客户端实现
// 支持标准 OpenAI API 和兼容接口（如灵娅AI）
...
type OpenAIConfig struct {
	...
	BaseURL string // 可选，自定义 API 端点（如灵娅AI）
}
...
func NewOpenAIClient(cfg OpenAIConfig) (*OpenAIClient, error) {
	...
	if cfg.Model == "" {
		cfg.Model = "gpt-4o-mini" // 默认使用经济型模型
	}
...
func (c *OpenAIClient) SummarizeDialogues(ctx context.Context, dialogues []*memoryModel.Memory) (*memory.TopicSummary, error) {
	if len(dialogues) == 0 {
...
func (c *OpenAIClient) ExtractProfile(ctx context.Context, topics []*memoryModel.Memory) (*memory.UserProfile, error) {
	if len(topics) == 0 {
...
func (c *OpenAIClient) AnalyzeIntent(ctx context.Context, userMessage string) (string, error) {
